...
function normalizeAddress(addr: string): string {
  return addr
    ...
    .replace(/Респ(?:ублика)?\.?\s+Башкортостан,?\s*/gi, '')
    .replace(/\bРБ\b,?\s*/g, '')
    // Убираем "городской округ X," и "г.о. X,"
    .replace(/городской\s+округ\s+[^,]+,?\s*/gi, '')
    .replace(/\bг\.?\s*о\.?\s+[^,]+,?\s*/gi, '')
    // Убираем приставку "г." / "г " перед названием города
    .replace(/^г\.?\s+/i, '')
    .replace(/(?<=,\s*)г\.?\s+/gi, '')
    // Убираем "село " / "с." в начале или после запятой если дублируется
